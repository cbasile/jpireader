...
JpiStream::JpiStream(const std::string& filename) : stream_(filename, std::ios::binary) {
  if (!stream_) {
...
uint8_t JpiStream::Read() {
  int c = stream_.get();
  if (c == EOF) {
    throw std::runtime_error("Unexpected EOF at counter " + std::to_string(counter_));
  }
...
void JpiStream::ResetCounter() {
  counter_ = 0;
}

int JpiStream::GetCounter() const {
  return counter_;
}

void JpiStream::ClearCurrentRecord() {
  current_record_.clear();
}

int JpiStream::GetCurrentRecordSize() const {
  return current_record_.size();
}

std::string JpiStream::GetCurrentRecordAsHexString() const {
  std::stringstream ss;
  for (size_t i = 0; i < current_record_.size(); ++i) {
    ss << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(current_record_[i]);
...
std::optional<std::string> JpiStream::GetChecksumFailureMessage() {
  ...
  if ((sum & 0xff) != 0) {
    std::stringstream ss;
    ss << "Checksum mismatch. Sum: " << std::hex << (sum & 0xff) << ". Record: " << GetCurrentRecordAsHexString();
    return ss.str();
  }
  return std::nullopt;
}

bool JpiStream::IsEndOfFile() {

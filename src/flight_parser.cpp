...
FlightParser::FlightParser(JpiStream& stream, const FlightMetadata& metadata, const MetadataUtil& metadata_util)
...
void FlightParser::ParseFlightHeader(Flight& flight) {
  ...
  if (flight.flight_number != flight_number_) {
    std::stringstream ss;
    ss << "Unexpected flight number " << flight.flight_number << " instead of " << flight_number_;
...
void FlightParser::ParseFlightData(Flight& flight) {
  ...
  while (stream_.GetCounter() + get_min_record_size() < estimated_flight_length_bytes_) {
    DataRecord data_record = parser.Parse(previous_data_record ? &*previous_data_record : nullptr);
  ...
  try {
    if (!metadata_util_.IsLastFlight(flight_number_)) {
      ...
      if (peek.size() >= 2) {
        int next_flight_number = metadata_util_.GetNextFlightNumber(flight_number_);
        if (((peek[0] << 8) | peek[1]) != next_flight_number) {
          if (peek.size() >= 3 && ((peek[1] << 8) | peek[2]) == next_flight_number) {
            stream_.Skip(1);
          }
        }
      }
